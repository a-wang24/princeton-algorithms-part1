...
public class Percolation {
  ...
  private final WeightedQuickUnionUF wquf, noBackwash;
  ...
  public void open(int row, int col) {
	  ...
	  if (row == 1) {
		  ...
		  noBackwash.union(index, topVirt);
	  } 
	  if (row == side) {
	  ...
	  if (col > 1 && isOpen(row, col-1)) {
		  ...
		  noBackwash.union(index, index-1);
	  } 
	  ...
	  if (row > 1 && isOpen(row-1, col)) {
		  ...
		  noBackwash.union(index, index-side);
	  }
  ...
  public boolean isFull(int row, int col) {
      validateInputs(row,col);
      return noBackwash.connected(xyto1D(row, col), topVirt);
  }
  ...
  /** converts row and column to 1D index */
